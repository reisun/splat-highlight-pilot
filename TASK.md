...
| # | タスク | 備考 |
|---|--------|------|
| 40 | ハイライト動画にイントロ演出を追加（オプション） | WebUIにタイトル入力欄を追加（任意）。有効時: ロゴズーム+タイトルカード(1-2秒)を動的生成 → 本編先頭の短区間(0.5秒程度)を切り出しイントロ末尾とxfadeで合成(再エンコード) → 残りの本編はcopy(無劣化)でconcat結合。ffprobeで入力動画のパラメータを取得しイントロを同一フォーマットで生成。ロゴ画像はassets/に配置。 |
...
| 39 | BUG-3: Phase Aカウントデータのタイムスタンプずれ修正 | [analyzer#77](https://github.com/reisun/splatoon-battle-analyzer/pull/77) |
